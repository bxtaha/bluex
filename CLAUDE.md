...
**The support widget is loaded on click, and that is not an optimisation.**
`components/ui/support-voice/` is the browser voice channel — a visitor clicks
a button and talks to the agent through their microphone, no phone call
involved. Importing `@elevenlabs/react` pulls in `@elevenlabs/client`, whose
entry point **statically** re-exports the WebRTC connection, which imports
`livekit-client`: ~300KB gzipped that `connectionType: "websocket"` does not
remove, because the import is not conditional. `support-panel.tsx` is behind a
`next/dynamic` boundary for that reason alone — **never import it from anything
that renders eagerly**, or the whole SDK lands on every page load and undoes
the entire performance pass above. Three gates decide whether anything loads:
the master toggle on the server in `app/(site)/layout.tsx` (off means it is
absent from the HTML, not hidden), the page rule in the browser via
`usePathname`, and the SDK on click.

**The audio worklets are served from `public/worklets/`, and the CSP depends on
it.** Left alone the SDK loads its two worklets from `blob:`/`data:` URLs and
fetches libsamplerate from `cdn.jsdelivr.net` — three additions to `script-src`
including a third-party host, on a site whose CSP comment correctly says it
loads no third-party script. Passing `workletPaths` and `libsampleratePath`
avoids all three, which is why the policy change this feature needed was two
lines: `microphone=(self)` and the provider's host in `connect-src`. The files
are vendored copies regenerated by `npm run worklets:sync`
(`scripts/sync-worklets.ts`) — **re-run it after upgrading `@elevenlabs/client`**,
because a stale copy fails as "unknown processor" at the moment somebody
clicks. The libsamplerate one is not a rare path: it loads whenever the device
sample rate differs from the agent's, which is the ordinary 48kHz-to-16kHz case.

**There is no live language detection, and the indicator's silence is correct.**
Nothing the SDK delivers carries a language — `conversation_initiation_metadata`
has a conversation id and two audio formats, and the `language_code` fields in
`@elevenlabs/types` belong to Scribe, a different product. The only runtime
signal is the agent's `language_detection` system tool being called, read off
`onAgentToolRequest`; it is **off by default and only the ElevenLabs dashboard
can enable it**. Showing the language from `overrides.agent.language` instead
would be displaying what we asked for as though it were what was heard. The
same shape of caveat applies to the greeting override: `overrides.agent.
firstMessage` is dropped silently unless first-message overrides are
allowlisted on that agent, which is also not settable from here.

**A conversation is filed by `channel`, not by a third `direction`.**
`lib/call-store.ts` carries `channel: "phone" | "web"`; a web conversation is
`direction: "inbound"` because the visitor came to us. Direction answers "who
dialled whom" and has no honest web value. `resolveRouting` in
`lib/call-intake.ts` is four ordered cases and only the last is new — it reads
the *absence* of `metadata.phone_call` rather than matching an agent id,
because the payload already answers the question and an id match would misfile
everything the moment a second agent were pointed at the widget. Rows written
before the field existed are phone calls, so the phone filter is `{ $ne: "web" }`
rather than `{ channel: "phone" }` — the other way round hides the entire
existing archive. Note that the collected fields (`analysis.data_collection_results`)
enrich a lead **only** on the web channel: `findOrCreateLeadByPhone` fills
blanks and never overwrites, so doing it for phone calls too would be safe and
arguably better, and was deliberately not done because that flow was out of
scope.
