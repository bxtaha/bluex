...
const glassButtonVariants = cva(
  "relative isolate cursor-pointer rounded-xl border-none bg-transparent p-0 outline-none transition-all",
  {
    variants: {
      size: {
...
const GlassButton = React.forwardRef<HTMLButtonElement, GlassButtonProps>(
  ({ className, children, size, contentClassName, ...props }, ref) => {
    return (
      <div
        className={cn(
          "glass-button-wrap relative inline-flex cursor-pointer rounded-xl",
          className,
        )}
      >
        <button
        ...
        <div className="glass-button-shadow rounded-xl" />
      </div>
    );
  },
);
